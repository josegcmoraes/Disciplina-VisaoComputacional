...
from skimage import data,color,img_as_float,measure,morphology,filters
import imageio
import os
...
for pasta in diretorio:
    ...
    for imagem in imagens_pasta:
        ...
        img = img_colorida.convert('L')
        '''
        img = img_as_float(img)
        masc = np.ones([5,5],dtype=float)
        masc = masc / 25
        media = filters.correlate(img,masc,mode='constant',cval=0)
        
        t_otsu = filter.threshold_otsu(img) 
        im_otsu = im < t_otsu 
        '''
        im.append(img)
        # plota a imagem
